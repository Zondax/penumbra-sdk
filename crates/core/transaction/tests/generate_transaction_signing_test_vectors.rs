...
use penumbra_sdk_asset::{asset, Value, ValueView};
...
use penumbra_sdk_keys::{Address, AddressView, FullViewingKey};
...
use penumbra_sdk_transaction::plan::{DetectionDataPlan, CluePlan};
    ...
    (
        ...
        .prop_map(
            |(proposal, vote, unbonded_amount, staked_note)| DelegatorVotePlan {
                proposal,
                vote,
                start_position: penumbra_sdk_tct::Position::from(rand::random::<u64>() % 100001),
                staked_note,
                unbonded_amount,
                position: penumbra_sdk_tct::Position::from(rand::random::<u64>() % 100001),
